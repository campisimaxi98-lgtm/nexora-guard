...
║                       "Un cerebro. Toda la inteligencia."                          ║
╚═══════════════════════════════════════════════════════════════════════════════════╝
```

[![CI](https://github.com/nexora-app-org/nexora-guard/actions/workflows/ci.yml/badge.svg)](https://github.com/nexora-app-org/nexora-guard/actions/workflows/ci.yml)
[![Release Android](https://github.com/nexora-app-org/nexora-guard/actions/workflows/release-android.yml/badge.svg)](https://github.com/nexora-app-org/nexora-guard/actions/workflows/release-android.yml)
...
📲 **[Descargar APK (último release) →](https://github.com/nexora-app-org/nexora-guard/releases/latest)**  ·  📘 **[Manual de usuario →](docs/MANUAL_USUARIO.md)** (qué es cada cosa, en claro)

---

**NEXORA GUARD es el hermano móvil de
[nexora-windows-inspector](https://github.com/nexora-app-org/nexora-windows-inspector):**
...
puede ser el primer indicio de que algo está ocurriendo.** Nexora vigila esas
...
> **Pendiente de recapturar.** Las capturas originales (`docs/images/`)
> pertenecían a la marca "RootCause" y no reflejan la identidad NEXORA.
> Se movieron a `docs/images/_old_rootcause_screenshots/` como referencia
> de layout, pero no se muestran acá para no confundir. No pude compilar
> ni correr la app en este entorno (sin SDK de Flutter/Android/Xcode), así
> que hacen falta capturas reales nuevas: `flutter run` en un emulador,
> capturar cada pestaña, y reemplazar los `<img>` de esta sección con
> `docs/images/*.png` actualizados. La landing (`landing/index.html`) ya
> tiene mockups ilustrativos con la identidad NEXORA a modo de placeholder.
...
El mismo principio que el rule engine del Nexora de escritorio, portado a
...
4. Abre **Nexora** y pulsa ↻ para tu primera captura.
...
| Perfil | Documento |
|---|---|
| 🧑‍💻 Desarrollador | [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) · [`docs/FLUTTER_PARA_NEXORA.md`](docs/FLUTTER_PARA_NEXORA.md) · [`docs/HEURISTICAS.md`](docs/HEURISTICAS.md) · [`docs/TESTING.md`](docs/TESTING.md) · [`docs/BUILD_MOVIL.md`](docs/BUILD_MOVIL.md) · [`docs/COMMANDS.md`](docs/COMMANDS.md) |
...
- **Colores oficiales**: dorado `#C9A24B` (NEXORA BRAIN, información clave),
  azul `#3A6EA5` (tecnología/protección), rojo `#C0392B` (peligro/alerta),
  sobre fondo `#0A0F24`. Definidos en `lib/ui/theme.dart`.
- **Ícono e identidad visual**: fuente de verdad en `assets/brand/*.svg`.
  Regenerar todos los formatos (Android, iOS, web) con:
  ```bash
  python scripts/make_app_icons.py
  ```
  Requiere `rsvg-convert` (paquete `librsvg2-bin`) e ImageMagick (`convert`).
- **Nombre del canal nativo**: `nexora/collectors` (antes `rootcause/collectors`
  en iOS — ya corregido; verificar que Dart, Kotlin y Swift coincidan si se
  vuelve a tocar `AppDelegate.swift`, `CollectorsChannel.kt` o `collectors.dart`).

## ✍️ Autor

Max · [@nexora-app-org](https://github.com/nexora-app-org)
