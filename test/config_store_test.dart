...
void main() {
  ...
  setUp(() async {
    tmp = await Directory.systemTemp.createTemp('nexora-config-test');
  });
  ...
  test('migra el archivo de idioma heredado de v0.1.x', () async {
    await File('${tmp.path}/nexora-language').writeAsString('en');
    final store = ConfigStore(tmp.path);
    ...
    expect(File('${tmp.path}/nexora-language').existsSync(), isFalse);
