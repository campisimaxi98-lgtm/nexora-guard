...
import 'package:nexora_guard/core/snapshot_json.dart';
...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('nexora-test-');
  });
  ...
  test('una línea corrupta se ignora sin perder el resto', () async {
    ...
    await store.append(capture(ts: 1));
    await File(
      '${tempDir.path}/nexora-history.jsonl',
  ...
  group('cadena de integridad (v0.5.0)', () {
    ...
    test('editar una captura a mano ROMPE la cadena (evidencia)', () async {
      ...
      final file = File('${tempDir.path}/nexora-history.jsonl');
