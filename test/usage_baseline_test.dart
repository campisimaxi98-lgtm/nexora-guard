...
void main() {
  ...
  setUp(() async {
    dir = await Directory.systemTemp.createTemp('nexora-usage-test');
  });
  ...
  Future<void> seed(
    ...
    await File('${dir.path}/nexora-usage-baseline.json').writeAsString(
      jsonEncode({
  ...
  test('en iOS (sin auditoría de apps) la regla no corre', () async {
    ...
    expect(anomalies, isEmpty);
    expect(
      await File('${dir.path}/nexora-usage-baseline.json').exists(),
      isFalse,
    );
  });
  ...
  test('el muestreo respeta el intervalo y no engorda el archivo', () async {
    ...
    final decoded =
        jsonDecode(
              await File(
                '${dir.path}/nexora-usage-baseline.json',
              ).readAsString(),
            )
