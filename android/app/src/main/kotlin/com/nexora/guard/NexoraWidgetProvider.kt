 ...
 * última línea del historial (`nexora-history.jsonl`) — sin servicios
 ...
 */
class NexoraWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        ...
        fun updateAll(context: Context) {
            try {
                ...
                val ids = manager.getAppWidgetIds(
                    ComponentName(context, NexoraWidgetProvider::class.java),
                )
                if (ids.isEmpty()) return

                val views = RemoteViews(context.packageName, R.layout.nexora_widget)
        ...
        private fun lastCapture(context: Context): JSONObject? = try {
            val file = File(context.filesDir, "nexora-history.jsonl")
            if (!file.exists()) {
                null
            } else {
                file.readLines()
        ...
        private fun widgetLang(context: Context): String {
            ...
            return try {
                val file = File(context.filesDir, "nexora-config.json")
