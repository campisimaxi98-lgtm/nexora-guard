...
| Baseline de apps vistas (para detectar instalaciones nuevas) | ✅ | En el sandbox (`nexora-apps-baseline.json`) | ❌ |
| Tiempo en pantalla por app (solo si TÚ concedes el acceso de uso en Ajustes) | ✅ opt-in | En historial local | ❌ (solo si exportas) |
| Cercanía histórica (días en que se vio cada BLE, opt-in) | ✅ opt-in | `nexora-nearby.json` (sandbox) | ❌ |
| Registro local de errores (para diagnóstico) | ✅ | `nexora-crashlog.txt` (sandbox) | ❌ (solo si lo compartes) |
...
2. **Historial en el sandbox**: `nexora-history.jsonl` vive en el
...
8. **El informe, el backup y el registro de errores se comparten por el
   ...
   que envía, no Nexora. La app nunca toca la red.
