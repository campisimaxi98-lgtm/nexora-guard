...
Este documento registra la decisión de licencia de NEXORA GUARD y el razonamiento detrás de ella.

## Licencia actual

**Apache License 2.0**, desde la fundación del proyecto (v0.1.0) y vigente en v0.5.0. El texto completo está en [`LICENSE`](../LICENSE).

Es la misma licencia que el proyecto hermano, [nexora-windows-inspector](https://github.com/nexora-app-org/nexora-windows-inspector), que documentó la decisión primero: misma familia, misma licencia. Este proyecto la hereda de forma consciente desde el primer commit, sin la etapa MIT inicial que tuvo la edición Windows.
...
- La **marca** y el nombre "Nexora" (eso requiere registro de marca separado).
...
La decisión de fondo (dual license vs mantener Apache 2.0 en una eventual v1.0) se evalúa a nivel de familia Nexora y está documentada en el `LICENCIA_Y_DECISION.md` del proyecto Windows. Mientras esa evaluación no ocurra, este proyecto permanece en Apache 2.0.
