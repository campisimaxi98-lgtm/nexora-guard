...
# Ej.: version: 0.5.0+7 — semver Y build number incrementados respecto al release anterior
```
...
```bash
gh release view v0.8.0 --json body --jq .body | grep -i "modo de firma"
```

- [ ] Hash SHA-256 verificado en local — debe coincidir carácter a carácter con `SHA256SUMS.txt`:

```powershell
gh release download v0.8.0 -p '*universal.apk' -p 'SHA256SUMS.txt' -D "$env:TEMP\rc-verify" --clobber
...
# Debe terminar en "Nexora abierto en el emulador" y la app debe arrancar sin crashes
```
