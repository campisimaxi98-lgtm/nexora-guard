...
NEXORA GUARD usa **Flutter** con una separación estricta en tres capas:
...
flowchart TB
    ...
    subgraph NATIVE["📟 Colectores nativos — MethodChannel 'nexora/collectors'"]
...
Canal: `nexora/collectors`. Métodos:
...
`nexora-config.json`) y la regla de tendencia consume el historial
...
`usage_baseline.dart` mantiene `nexora-usage-baseline.json`: una muestra
...
| Archivo | Qué guarda |
|---|---|
| `nexora-history.jsonl` | Capturas selladas con la cadena SHA-256 |
| `nexora-config.json` | Idioma, modo de vista, intervalos y umbrales |
| `nexora-apps-baseline.json` | Apps conocidas, versión, permisos y `createdMillis` |
| `nexora-usage-baseline.json` | Series de consumo por app (v0.8.0) |
| `nexora-crashlog.txt` | Registro local de errores no capturados |
...
| Ids de hallazgo estables | Evidencia comparable entre dispositivos y con Nexora Windows |
...
En **consumo en ejecución** la app sigue la filosofía Nexora: sin red y
