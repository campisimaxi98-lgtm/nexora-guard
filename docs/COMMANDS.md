...
adb uninstall com.nexora.guard            # desinstalar (borra historial y exports)
...
```bash
adb logcat --pid="$(adb shell pidof -s com.nexora.guard)"
...
```bash
adb shell screencap -p /sdcard/nexora.png
adb pull /sdcard/nexora.png
adb shell rm /sdcard/nexora.png
```
...
gh release download -R nexora-app-org/nexora-guard -p '*arm64-v8a.apk'
gh release download -R nexora-app-org/nexora-guard -p '*universal.apk'
gh release download -R nexora-app-org/nexora-guard -p 'SHA256SUMS.txt'
