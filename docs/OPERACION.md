...
Guía de operación día a día de NEXORA GUARD: cómo capturar,
...
que el Nexora de escritorio— y opcionalmente en segundo plano con la
...
2. lo guarda como `nexora-snapshot-<timestamp>.json` en el sandbox de la app.
...
|---|---|---|
| Historial (máx. 500 capturas) | `nexora-history.jsonl` | Sandbox de la app |
| Exports JSON | `nexora-snapshot-<timestamp>.json` | Sandbox de la app |
| Configuración (idioma, intervalos, umbrales) | `nexora-config.json` | Sandbox de la app |
| Baseline de apps vistas (detección de instalaciones) | `nexora-apps-baseline.json` | Sandbox de la app |

El sandbox es el directorio privado de la app (en Android, el `filesDir` de
`com.nexora.guard`; en iOS, su carpeta Documents). Ninguna otra
