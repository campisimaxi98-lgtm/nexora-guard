...
`%USERPROFILE%\.keystores\nexora-mobile.jks` — credenciales en
`nexora-mobile-keystore-info.txt`. Sube los 4 secretos al repo:

```bash
# En PowerShell, desde una máquina con el keystore:
$b64 = [Convert]::ToBase64String([IO.File]::ReadAllBytes("$env:USERPROFILE\.keystores\nexora-mobile.jks"))
gh secret set ANDROID_KEYSTORE_BASE64  -R nexora-app-org/nexora-guard -b $b64
gh secret set ANDROID_KEYSTORE_PASSWORD -R nexora-app-org/nexora-guard -b "<store-pass>"
gh secret set ANDROID_KEY_ALIAS         -R nexora-app-org/nexora-guard -b "nexora"
gh secret set ANDROID_KEY_PASSWORD      -R nexora-app-org/nexora-guard -b "<key-pass>"
```

Verifica que quedaron los cuatro:

```bash
gh secret list -R nexora-app-org/nexora-guard
```
...
- El envío se hace vía merge request a `fdroiddata`. Documentar aquí el
  identificador de la app: `com.nexora.guard`.
